...
class StatusController extends Controller
{
    ...
    public function show($id)
    {
       ...
       if($id == Auth::id() || Auth::user()->isAdmin())
       {
           ...
           $refers =$refers_list->getRefers($id,0);
         
           AlertsPays::referAmountLevel($refers,$id);
           ...
           $pay = $investments->where('user_id','=',$id)->where('state',$range_name->range)->latest()->exists();
            ...
            if($pay)
            {
                ...
                if(count($refers) == 7 && count($refers[7])==128)
                {
                    ...
                    {
                        $investment = Investment::where('user_id',$user->user_id)->where('state','=',$range_name->range)->exists();
                       if($investment)
                       {
                            $iterator++;
                       }
                    ...
                    { 
                        ...
                        $user_id=Auth::id();
                        $verified_pay = PaysCompleted::where('user_id',$user_id)->where('level_pay','Nivel 7')->where('range_id',$range)->exists();
                        if($verified_pay)
                        {
                        ...
                        $refers_list->where('sponsor_id','=',$id)->delete();
                        $commissions->where('user_id','=',$id)->delete();
